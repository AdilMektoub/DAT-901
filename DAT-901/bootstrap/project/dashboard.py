...
def saveRFM(data):
    ...
    # rfm.insert(column="CLI_ID", loc=0,
    #            value=filtered_data["CLI_ID"].unique())
    rfm.rename(columns={
        "PRIX_NET": "monetary",
        "TICKET_ID": "frequency",
        "MOIS_VENTE": "recency"
    ...
    rfm_display.to_csv("RFM.csv", sep=",", index=False)
...
def importProducts():
    try:
        products = pd.read_csv("Products_ids.csv", sep=",")
    except:
        save_products()
        products = pd.read_csv("Products_ids.csv", sep=",")
...
display = st.sidebar.radio("View: ", ("All Data", "Search Client"))
...
if display == "All Data":
    ...
    client_id = st.number_input("Client ID", value=0)
    if client_id != 0:
        df_client = df.copy()
        ...
        if len(df_client.index) > 0:
            ...
            with st.expander("Client Informations", expanded=False):
                ...
                model = pickle.load(
                    open("models{}model_spotlight.pkl".format(os.sep), "rb"))
            ...
            with st.expander("Client History", expanded=False):
                df_client = df_client.drop("QUANTITY", axis=1)
                st.write(df_client)
